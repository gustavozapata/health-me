                    ...
                    VStack{
                        if msg.isUser {
                            ...
                            VStack(alignment: .leading, spacing: 10) {
                                ...
                                Text(msg.text).padding().background(Color.messageBg).cornerRadius(16).lineSpacing(5)
...
struct MessagesDetail_Previews: PreviewProvider {
    static var previews: some View {
        return MessagesDetail(messages: [MessageModel(date: Date(), time: "12:00", text: "Message", isUser: false, options: [MessageOptionModel(code: 100, option: "Cancel the appointment")])]).preferredColorScheme(.dark)
    }
}
